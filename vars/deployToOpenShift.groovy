def call(Map config) {
    withCredentials([string(credentialsId: 'openshift-cred-id', variable: 'OPENSHIFT_TOKEN')]) {
        sh """
            ...
            sed 's|\${DOCKER_REGISTRY}|'${config.registry}'|g; s|\${DOCKER_IMAGE}|'${config.imageName}'|g; s|\${BUILD_NUMBER}|'${config.buildNumber}'|g' app-deployment.yaml > deployment_processed.yaml
